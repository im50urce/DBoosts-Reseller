...
async def latest_version_check():
    try:
        async with httpx.AsyncClient() as client:
            github_data = await client.get("https://api.github.com/repos/im50urce/DBoosts-Reseller/releases/latest")
...
async def startup():
    ...
    print("Coded with <3 by 50urce | https://github.com/im50urce/DBoosts-Reseller")
    ...
    print("\nConnecting to DBoosts API...")
    api = luxurynitro.Client(config.api_key)
    try: api_user = await api.get_user()
    except luxurynitro.errors.APIError as exc:
        print(f"Error connecting to DBoosts API ({api._base_url}): {exc.message}")
        if exc.response.status_code == 401:
            print("It seems like your DBoosts API key is invalid. You can get a new one at https://dboosts.cc/settings")
